    ...
    <div className="flex flex-col gap-6 md:gap-8">
                ...
                <div>
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            ...
                            className="bg-brand-green-light text-brand-green-dark font-semibold text-xs px-3 py-1.5 rounded-full hover:bg-brand-green/20 transition-colors flex items-center gap-1.5 theme-athlete:bg-zinc-700 theme-athlete:text-zinc-300 theme-athlete:hover:bg-zinc-600"
                        >
      ...
      <div className="bg-white/80 backdrop-blur-sm p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-2">
          ...
          <div className="w-full bg-gray-200 rounded-full h-2 theme-athlete:bg-slate-700">
              <div className="bg-blue-500 h-2 rounded-full" style={{ width: `${weightProgress}%` }}></div>
          </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-2 lg:grid-cols-4 md:gap-6">
        ...
        <InfoCard 
            icon={<WaterDropletsIcon className="text-sky-500"/>} 
        ...
        >
           ...
           <div className="flex gap-2 mt-3">
            <button onClick={() => addWater(0.250)} className="flex-1 text-xs bg-white border border-gray-300 hover:bg-gray-100 text-slate-700 font-semibold py-2 px-2 rounded-lg transition-colors">+250ml</button>
            <button onClick={() => addWater(0.500)} className="flex-1 text-xs bg-white border border-gray-300 hover:bg-gray-100 text-slate-700 font-semibold py-2 px-2 rounded-lg transition-colors">+500ml</button>
           </div>
        </InfoCard>

        <InfoCard 
            icon={<UtensilsIcon className="text-purple-600"/>} 
        ...
        <InfoCard 
            icon={<FireIcon className="text-brand-orange"/>} 
            iconBg="bg-brand-orange-light"
        >
            <h3 className="text-slate-500 font-medium text-sm md:text-base">Calorias</h3>
            <p className="text-2xl md:text-3xl font-bold my-1 text-slate-800">{macros.calories.current}
                <span className="text-base md:text-xl text-slate-400"> / {macros.calories.goal}kcal</span>
            </p>
            <div className="w-full bg-gray-200 theme-athlete:bg-slate-700 rounded-full h-2 mt-2">
                <div className="bg-brand-orange h-2 rounded-full" style={{ width: `${Math.min((macros.calories.current / macros.calories.goal) * 100, 100)}%` }}></div>
            </div>
        </InfoCard>
        
        <InfoCard 
            icon={<QuestionMarkCircleIcon className="text-purple-600"/>} 
            iconBg="bg-purple-100"
        >
            <h3 className="text-slate-500 font-medium text-sm md:text-base">Ajuda & Tutorial</h3>
            <p className="text-slate-400 text-sm my-1 leading-tight">Não sabe por onde começar? Refaça nosso tour guiado.</p>
            <button 
        ...
        >
            <h3 className="text-slate-500 font-medium mb-3">Macronutrientes</h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-x-6 gap-y-4">
                ...
                <div>
                    ...
                    <div className="w-full bg-gray-200 theme-athlete:bg-slate-700 rounded-full h-2 mt-1"><div className="bg-emerald-500 h-2 rounded-full" style={{ width: `${Math.min((macros.protein.current / macros.protein.goal) * 100, 100)}%` }}></div></div>
                </div>
                <div>
                    <div className="flex justify-between items-center text-sm">
                    ...
                    <div className="w-full bg-gray-200 theme-athlete:bg-slate-700 rounded-full h-2 mt-1"><div className="bg-amber-500 h-2 rounded-full" style={{ width: `${Math.min((macros.fat.current / macros.fat.goal) * 100, 100)}%` }}></div></div>
                </div>
            </div>
        </InfoCard>
      </div>
