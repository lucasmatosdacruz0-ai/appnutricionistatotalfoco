...
const ProgressView: React.FC<{ userData: UserData; handlers: UserDataHandlers; }> = ({ userData, handlers }) => {
    ...
    const getRemainingUses = (featureKey: string) => {
        ...
        // FIX: Ensure correct usage data is accessed based on feature.period and explicitly cast to Number
        const currentUsage = Number(feature.period === 'week' 
            ? (userData.weeklyUsage as any)[featureKey] 
            : (userData.dailyUsage as any)[featureKey]) || 0;
        const purchasedUsage = Number(userData.purchasedUses?.[featureKey]) || 0;
